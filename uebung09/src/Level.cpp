...
{   
    /*
      Ich hab in WorldProperty.cpp das m_damping(0.9,1.0) auf m_damping(0.2,1.0) verändert
      da der Player sonst zu schnell lief.
     */
	...
	{
	   fall = m_levelPhysics.gravity() * dt;
	}
	
	double d = m_player->physics().maxFallVelocity();
	if(fall.y() > d)
	{
	    fall.setY(d);
	}
	...
	Vector2f run =  m_player->physics().moveForce() * dt * move;
	
	double d2 = m_player->physics().maxRunVelocity();
	if(run.x() > d2)
	{
	    run.setX(d2);
	}

	// Update players velocity vector
	m_player->physics().setVelocity(fall + run);

	// Add jumping momentum
	if(m_player->jumping())
	{
	    Vector2f jump = m_player->physics().jumpForce() * dt;
	    m_player->physics().setVelocity(jump + run);
	}

	// Damp velocity according to extrinsic level damping
	Vector2f damp = m_levelPhysics.damping();
	damp *= m_player->physics().velocity();
	damp = m_player->physics().velocity() - damp;

	// Clamp velocities
	Vector2f velo = m_player->physics().velocity() - damp;

	// Set new player position
	m_player->setPosition(velo + m_player->position());
